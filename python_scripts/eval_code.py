...
def parse_args():
    ...
    parser.add_argument(
        "--enc_tokenizer",
        default="None",
        help="tokenizer of the encoder if using langbridge"
    )
    parser.add_argument(
        "--modeltype",
        default="causal",
...
def main():
    ...
    load_data_paths = args.load_data_path.split(",")

    assert len(load_data_paths) == len(
        task_names), "Number of datasets must match number of tasks"
    ...
    if args.load_generations_path:
        ...
        elif args.modeltype == 'langbridge':
        ...
        if args.modeltype == 'langbridge':
            ...
            tokenizer_enc = AutoTokenizer.from_pretrained(
                args.enc_tokenizer,
                use_fast=False,
                truncation_side="left",
            )
        ...
        evaluator = Evaluator(accelerator, model,
                              tokenizer, tokenizer_enc, args)

        for task, path in zip(task_names, load_data_paths):
            ...
            # else:
            task_name, lang = task.split("#")
            results[task] = evaluator.evaluate(task_name, path)
